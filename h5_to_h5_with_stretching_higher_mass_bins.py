...
parser.add_argument('--input_file', default='/global/cfs/cdirs/m4392/bbbam/IMG_aToTauTau_mNeg1p2T018_unbiased_original_combined_with_stretching_lower_unphy_bins_h5/IMG_aToTauTau_Hadronic_m1p2To18_pt30T0300_unbiased_original_combined_with_stretching_unphy_bins_train.h5',
                    help='input data path')
parser.add_argument('--output_data_path', default='/global/cfs/cdirs/m4392/bbbam/IMG_aToTauTau_mNeg1p2T018_unbiased_original_combined_with_stretching_lower_unphy_and_upper_mass_bins_h5',
...
outfile = f'{prefix}_with_stretching_higher_mass_bins.h5'
...
with h5py.File(f'{outdir}/{outfile}', 'w') as proper_data:
    ...
    for start_idx_ in tqdm(range(0, num_images, batch_size)):
        ...
        lower_mass_mask = am_batch <= 14
        upper_mass_mask = am_batch > 14
        ...
        lower_am_batch = am_batch[lower_mass_mask].reshape(-1, 1)
        lower_ieta_batch = ieta_batch[lower_mass_mask].reshape(-1, 1)
        lower_iphi_batch = iphi_batch[lower_mass_mask].reshape(-1, 1)
        lower_apt_batch = apt_batch[lower_mass_mask].reshape(-1, 1)

        upper_images_batch = images_batch[upper_mass_mask.flatten()]
        upper_am_batch = am_batch[upper_mass_mask]
        upper_ieta_batch = ieta_batch[upper_mass_mask]
        upper_iphi_batch = iphi_batch[upper_mass_mask]
        upper_apt_batch = apt_batch[upper_mass_mask]
        # print("len(lower_images_batch)", len(lower_images_batch))
        # print("len(upper_images_batch)", len(upper_images_batch))
        if (len(upper_images_batch)) > 0:
            new_am_batch = (new_mass(upper_am_batch)).reshape(-1, 1)
            new_images_batch = upper_images_batch
            new_ieta_batch = upper_ieta_batch.reshape(-1, 1)
            new_iphi_batch = upper_iphi_batch.reshape(-1, 1)
            new_apt_batch = upper_apt_batch.reshape(-1, 1)

        if ((len(lower_images_batch) > 0) and (len(upper_images_batch) > 0)):
            full_am_batch = np.concatenate([lower_am_batch, new_am_batch], axis=0)
            full_images_batch = np.concatenate([lower_images_batch, new_images_batch], axis=0)
            full_ieta_batch = np.concatenate([lower_ieta_batch, new_ieta_batch], axis=0)
            full_iphi_batch = np.concatenate([lower_iphi_batch, new_iphi_batch], axis=0)
            full_apt_batch = np.concatenate([lower_apt_batch, new_apt_batch], axis=0)
